...
The original design assumed `figment` for layered env + `.env`
configuration. The actual env-var namespace is non-uniform
...
purpose, decided before config loading was implemented so the dependency
table doesn't carry an unused `figment` entry.
