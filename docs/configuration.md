...
| `REDMINE_MCP_MAX_RESPONSE_ITEMS` | no | `200` | Caps the item count in any list-shaped tool response. A positive integer; `0` is `Invalid`. A truncated response reports it via `pagination.truncated` and a hint rather than a silent cut. |
| `REDMINE_MCP_MAX_RESPONSE_BYTES` | no | `262144` (256 KiB) | Caps the serialized byte size of any tool response, applied together with the item cap above. A positive integer; `0` is `Invalid`. |
...
| `REDMINE_PRODUCTS_ENABLED` | no | `false` | `plugin_flags.products`. `true` registers `manage_product` (RedmineUP Products plugin); `false` (the default) de-registers it from the router entirely — absent from `tools/list`, and `tools/call` fails with "tool not found", not an in-band error. |
